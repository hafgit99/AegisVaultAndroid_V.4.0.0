...
export const BackupModal = ({
    ...
    try {
      ...
      const path =
        formatId === 'csv'
          ...
          : formatId === 'canonical_json'
          ? await BackupModule.exportCanonicalJSON()
  ...
  const handleImport = async (source: ImportSource) => {
    try {
      ...
      // Check if encrypted Aegis file (any source, any extension)
      // Desktop backup: format='aegis-encrypted-v1', Android: encrypted=true
      try {
        const rawContent = await RNFS.readFile(filePath, 'utf8');
        try {
          const jsonCheck = JSON.parse(rawContent);
          const isAndroidEncrypted = !!(jsonCheck.encrypted && jsonCheck.data);
          const isDesktopEncrypted = !!(
            (jsonCheck.format === 'aegis-vault-backup' || jsonCheck.format === 'aegis-encrypted-v1') &&
            jsonCheck.payload &&
            jsonCheck.salt &&
            jsonCheck.iv
          );
          if (isAndroidEncrypted || isDesktopEncrypted) {
            console.log('[BackupModal] Encrypted backup detected:', {
              isAndroidEncrypted,
              isDesktopEncrypted,
              format: jsonCheck.format,
              hasPayload: !!jsonCheck.payload,
              hasEncrypted: !!jsonCheck.encrypted,
            });
          ...
          // Not JSON - continue with normal import
        }
      } catch (readErr) {
        console.log('[BackupModal] File read error:', readErr);
      }
            ...
            <View style={st.modalButtonRow}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  ...
                  } else if (pendingPlainExport === 'canonical_json') {
                    continuePlaintextExport('canonical_json');
                  }
                }}
                activeOpacity={0.7}
              >
