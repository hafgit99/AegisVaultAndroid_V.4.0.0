          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            >
              <Text style={[s.label, { color: cc.muted }]}>
                {t('donation.select_coin')}
              </Text>
              <View style={s.assetContainer}>
                {(
                  Object.keys(CRYPTO_ADDRESSES) as Array<
                    keyof typeof CRYPTO_ADDRESSES
                  >
                ).map(key => (
                  <TouchableOpacity
                    key={key}
                    style={[
                      ...
                      selected === key && [
                        s.assetChipActive,
                        { backgroundColor: cc.sage, borderColor: cc.sage },
                      ],
                    ]}
                    onPress={() => {
                      setSelected(key);
                  ...
                  >
                    <Text
                      style={[
                        s.assetText,
                        { color: cc.navy },
                        selected === key && s.assetTextActive,
                      ]}
                    >
            ...
            >
              <View style={s.addressRow}>
                <Text style={[s.addressTitle, { color: cc.navy }]}>
                  {t(`donation.assets.${selected}`)}
                </Text>
                <TouchableOpacity
                  onPress={() => setShowQR(!showQR)}
                  style={[s.qrToggle, { backgroundColor: cc.sageLight }]}
                >
                  <Text style={[s.qrToggleText, { color: cc.sage }]}>
              ...
              >
                <Text style={[s.addressText, { color: cc.navy }]} selectable>
                  {address}
                </Text>
              </View>
              ...
              <TouchableOpacity
                style={[
                  ...
                  copied && [s.copyBtnSuccess, { backgroundColor: cc.green }],
                ]}
                onPress={copy}
                activeOpacity={0.7}
              >
                <Text style={s.copyBtnText}>
                  {copied
                    ? `✓ ${t('donation.addr_copied')}`
                    : `📋 ${t('donation.copy_addr')}`}
                </Text>
              </TouchableOpacity>
            </View>
